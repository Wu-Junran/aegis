...
export function processTextPrompt(
  ...
  // [M4] PHI gate: in strict/research mode, never export the raw user prompt
  // to the interaction span (Perfetto/OTel attributes + addBetaInteractionAttributes
  // — see startInteractionSpan), since the prompt has not yet been through the
  // de-id middleware at this point in the pipeline. The keyword-match logic
  // below uses `userPromptText` locally and never escapes the process — that
  // stays raw. Only the telemetry-bound version is replaced. (The user_prompt
  // OTel event is gated centrally inside `redactIfDisabled` — see events.ts.)
  const interactionPrompt =
    getPhiMode() === 'off' ? userPromptText : '<REDACTED>'
  startInteractionSpan(interactionPrompt)
  ...
  // unchanged for keyword matching below to preserve existing semantics.
