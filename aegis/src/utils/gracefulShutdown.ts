...
export const setupGracefulShutdown = memoize(() => {
  ...
  // Hammer escape hatch — second shutdown signal SIGKILLs the process.
  //
  // Why this exists: gracefulShutdown's failsafe timer (line 430) is a
  // setTimeout, which schedules into libuv's TIMERS phase. Timers only fire
  // after the PENDING phase drains. If a stream-shutdown callback in the
  // pending queue keeps re-queuing itself (the libuv `AfterUvShutdown.cold.1`
  // re-entry pattern observed at 100% CPU on Node 25 + heavy-stdio configs —
  // many MCP stdio pipes + raw-mode TTY + alt-screen on shutdown), the timers
  // phase never runs and the failsafe never fires. The process spins forever.
  //
  // Without this hammer, `if (shutdownInProgress) return` at the top of
  // gracefulShutdown means a second Ctrl+C is a no-op — the user has no
  // manual escape. With it, the second signal calls process.kill(SIGKILL),
  // which the kernel processes DIRECTLY, outside the event loop. SIGKILL
  // is uncatchable and works even when uv_run is wedged in pending.
  //
  // Pattern matches npm / yarn / dockerd / kubectl / many CLI tools.
  // Logging is best-effort: if stderr is itself part of the wedged stream
  // set, the writeSync may no-op or throw; we always reach the kill() call.
  ...
  function handleShutdownSignal(
    ...
    logForDiagnosticsNoPII('info', 'shutdown_signal', {
      signal,
      hammer_count: shutdownSignalCount,
    })
    if (shutdownSignalCount >= 2) {
      // Second signal: graceful path is wedged. Force-kill via the kernel.
      try {
        writeSync(
          2,
          `\nshutdown wedged (second ${signal}); force-killing pid ${process.pid}\n`,
        )
      } catch {
        // stderr may be in the wedged stream set — ignore and proceed to kill.
      }
      try {
        process.kill(process.pid, 'SIGKILL')
      } catch {
        // process.kill can throw on certain platforms when the target is
        // self and a zombie state is reached; fall through to process.exit
        // as a last resort. process.exit may also block on flushes, but
        // we've already attempted SIGKILL — nothing more we can do here.
      }
    ...
    if (
      ...
      // Note: we DECREMENT the counter so a print-mode SIGINT we ignored
      // doesn't pre-arm the hammer for the user's first interactive Ctrl+C.
      shutdownSignalCount--
      return
    }
  ...
  process.on('SIGINT', () => handleShutdownSignal('SIGINT', 0))
  process.on('SIGTERM', () => handleShutdownSignal('SIGTERM', 143))
  if (process.platform !== 'win32') {
    process.on('SIGHUP', () => handleShutdownSignal('SIGHUP', 129))
  ...
  if (process.env.AEGIS_INPUT_DIAG) {
    try {
      ...
      nodeFs.appendFileSync(
        ...
        `[${new Date().toISOString()}] gracefulShutdown(code=${exitCode}, reason=${reason}, finalMessage=${JSON.stringify(options?.finalMessage ?? null)}) called\n    ${stack}\n`,
      )
    } catch {
      // swallow
    }
