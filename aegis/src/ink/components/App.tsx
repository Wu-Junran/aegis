...
// AEGIS opt-in diagnostic: when AEGIS_INPUT_DIAG is set, monkey-patch
// stdin.on/addListener/once to log every event subscription + the call
// site. Also spawns a 500ms poll of listener counts and flowing state.
// Runs once at module load; cheap no-op if env var is absent.
if (process.env.AEGIS_INPUT_DIAG) {
  try {
    // eslint-disable-next-line @typescript-eslint/no-require-imports, custom-rules/no-sync-fs
    const fs = require('node:fs') as typeof import('node:fs');
    const log = (msg: string): void => {
      try {
        fs.appendFileSync('/tmp/aegis-input-diag.log', `${msg}\n`);
      } catch {
        // swallow
      }
    };
    log(`=== AEGIS_INPUT_DIAG session start ${new Date().toISOString()} ===`);
    // Cast to any to avoid Node's heavily-overloaded stream typings; this
    // block is diagnostic-only and opt-in.
    // biome-ignore lint/suspicious/noExplicitAny: diagnostic override
    const stdinAny = process.stdin as any;
    // biome-ignore lint/suspicious/noExplicitAny: diagnostic override
    const wrap = (name: string, orig: any) =>
      // biome-ignore lint/suspicious/noExplicitAny: diagnostic override
      (event: any, ...args: any[]) => {
        const stack = new Error().stack?.split('\n').slice(2, 4).map((s: string) => s.trim()).join(' <- ') ?? '?';
        log(`[${new Date().toISOString()}] stdin.${name}(${String(event)}) call ${stack}`);
        return orig.call(stdinAny, event, ...args);
      };
    stdinAny.on = wrap('on', stdinAny.on);
    stdinAny.addListener = wrap('addListener', stdinAny.addListener);
    stdinAny.once = wrap('once', stdinAny.once);
    setInterval(() => {
      const data = stdinAny.listenerCount('data');
      const readable = stdinAny.listenerCount('readable');
      const flowing = stdinAny._readableState?.flowing;
      const paused = stdinAny.isPaused?.();
      log(`[${new Date().toISOString()}] poll data=${data} readable=${readable} flowing=${flowing} paused=${paused}`);
    }, 500);
    // Catch unexpected process.exit() calls to surface early terminations.
    // biome-ignore lint/suspicious/noExplicitAny: diagnostic override
    const procAny = process as any;
    const origExit = procAny.exit;
    procAny.exit = (code?: number) => {
      const stack = new Error().stack?.split('\n').slice(2, 6).map((s: string) => s.trim()).join('\n    ') ?? '?';
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports, custom-rules/no-sync-fs
        require('node:fs').appendFileSync(
          '/tmp/aegis-input-diag.log',
    ...
    process.on('beforeExit', (code) => {
      log(`[${new Date().toISOString()}] beforeExit code=${code}`);
    });
...
export default class App extends PureComponent<Props, State> {
  ...
  // In test environments (NODE_ENV=test) the escape-sequence flush delay is
  // dropped to 0 so that a lone \x1b is resolved as Escape within a single
  // event-loop tick. Tests that send Esc via stdin.write('\x1b') and then
  // await a short Promise.resolve() or setTimeout(r,10) still see key.escape
  // fire, mirroring ink-testing-library + published ink's immediate-Esc
  // contract. NORMAL_TIMEOUT is only 50ms in production where the terminal
  // may follow a standalone ESC with a continuation byte (e.g. arrow key).
  readonly NORMAL_TIMEOUT = process.env.NODE_ENV === 'test' ? 0 : 50;
  ...
  handleReadable = (): void => {
    // AEGIS opt-in diagnostic: when AEGIS_INPUT_DIAG=1 is set, append raw
    // stdin bytes to /tmp/aegis-input-diag.log so input issues can be
    // diagnosed without rebuilding the bundle. No-op otherwise.
    if (process.env.AEGIS_INPUT_DIAG) {
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports, custom-rules/no-sync-fs
        require('node:fs').appendFileSync(
          '/tmp/aegis-input-diag.log',
          `[${new Date().toISOString()}] handleReadable fired, rawModeCount=${this.rawModeEnabledCount}\n`,
        );
      } catch {
        // swallow
      }
      ...
      while ((chunk = this.props.stdin.read() as string | null) !== null) {
        if (process.env.AEGIS_INPUT_DIAG) {
          try {
            ...
            require('node:fs').appendFileSync(
              '/tmp/aegis-input-diag.log',
              `  bytes: ${JSON.stringify([...String(chunk)].map((c) => c.charCodeAt(0)))}\n`,
            );
          } catch {
            // swallow
          }
