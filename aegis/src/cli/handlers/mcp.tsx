...
export async function mcpResetChoicesHandler(): Promise<void> {
  ...
  // Clear the legacy project-config location for users who never migrated.
  ...
  // Clear the authoritative location. The approval dialogs
  // (MCPServerApprovalDialog / MCPServerMultiselectDialog) and the
  // migration (migrateEnableAllProjectMcpServersToSettings) write the
  // approval state here, and getProjectMcpServerStatus() reads it from
  // the merged settings view. Without this, the command's promise to
  // re-prompt on next launch is silently broken.
