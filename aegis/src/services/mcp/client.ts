...
// Tools in this list are reachable ONLY via slash commands, never by the LLM.
// Rationale: spec §6.3 — PHI-ingress boundary. Loading a bundle must be an
// intentional user action; an LLM cannot exfiltrate a patient record by
// asking the tool to load an arbitrary file path.
const AEGIS_COMMAND_ONLY_TOOLS: ReadonlySet<string> = new Set([
  'mcp__aegis-mcp__fhir_load_bundle',
])

export function isAegisCommandTool(fullyQualifiedName: string): boolean {
  return AEGIS_COMMAND_ONLY_TOOLS.has(fullyQualifiedName)
}

/**
 * Apply all Aegis-specific MCP tool visibility filters. Exported so tests
 * exercise the exact function the production tool-fetch path runs.
 */
export function applyAegisMcpToolFilters(tools: readonly Tool[]): Tool[] {
  return tools
    .filter(isIncludedMcpTool)
    .filter(tool => !isAegisCommandTool(tool.name))
}
...
export const fetchToolsForClient = memoizeWithLRU(
  async (client: MCPServerConnection): Promise<Tool[]> => {
    ...
    try {
      ...
      return applyAegisMcpToolFilters(mapped)
    } catch (error) {
