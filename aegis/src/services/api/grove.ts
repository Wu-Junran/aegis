...
// Per-request HTTP deadline for Grove probes. axios's default `timeout: 0`
// means "wait forever", so without this any startup-time call into Grove
// would hang the REPL launch indefinitely if the network blackholes
// `api.anthropic.com` (offline / firewalled / non-Anthropic deployment).
// Aegis's medical-mode network policy (phiMode.ts) short-circuits Grove
// before this code runs, but an explicit operator opt-in to nonessential
// traffic in medical mode — or running with `--phi-mode=off` — would
// otherwise still hit the no-timeout hang. Bound it here to defense-in-
// depth the env-gate.
const GROVE_REQUEST_TIMEOUT_MS = 5_000
      ...
      const response = await withOAuth401Retry(() => {
        ...
        return axios.get<AccountSettings>(
          `${getOauthConfig().BASE_API_URL}/api/oauth/account/settings`,
          {
            ...
            timeout: GROVE_REQUEST_TIMEOUT_MS,
          },
        )
      })
