...
import { setSessionProvider } from './sessionProvider.js'

/**
 * Mirrors `AppState.currentProvider` into the process-local sessionProvider
 * holder. This is the ONLY production caller of `setSessionProvider` — the
 * lint-equivalent test in `sessionProvider-callers.test.ts` enforces that.
 *
 * Returns the unsubscribe function. Mount this once per process lifetime
 * via `<MedicalProviderBridge />`.
 */
export function wireSessionProviderFromAppState(
  store: Store<AppState>,
): () => void {
  let lastSeen = store.getState().currentProvider
  setSessionProvider(lastSeen) // pick up initial value
  const unsubscribe = store.subscribe(() => {
    const next = store.getState().currentProvider
    if (next !== lastSeen) {
      lastSeen = next
